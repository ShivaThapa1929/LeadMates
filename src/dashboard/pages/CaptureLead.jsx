...
import {
    UserPlus, Mail, Phone, ChevronRight,
    Sparkles, Shield, AlertCircle, CheckCircle,
    Loader2, Plus, X, Download, FileText
} from "lucide-react";
...
const CaptureLead = () => {
    ...
    const handleAddField = async (e) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                loadFields(); // Refresh the fields list
            }
        } catch (err) {
            addNotification({
                title: "Error",
    ...
    const handleDownloadPDF = (data = formData) => {
        ...
        const content = `
            ...
            <html>
            <head>
                <title>Lead Intelligence Report - ${data.name}</title>
                <style>
                    ...
                    .section-title { font-size: 12px; font-weight: 900; color: #3b82f6; text-transform: uppercase; margin-bottom: 15px; border-left: 4px solid #3b82f6; padding-left: 10px; }
                    .grid { display: grid; grid-template-cols: 1fr 1fr; gap: 20px; }
                    .field { background: #1e293b; padding: 15px; rounded: 12px; border: 1px solid #334155; }
            ...
            <body>
                <div class="header">
                    <div class="title">Lead Intelligence Report</div>
                    <div class="meta">Generated by LeadMates Mainframe | ${timestamp}</div>
                </div>
                
                <div class="section">
                    <div class="section-title">Core Entity Identity</div>
                    <div class="grid">
                        <div class="field"><div class="label">Name</div><div class="value">${data.name || 'N/A'}</div></div>
                        <div class="field"><div class="label">Email Address</div><div class="value">${data.email || 'N/A'}</div></div>
                        <div class="field"><div class="label">Contact Number</div><div class="value">${data.contact || 'N/A'}</div></div>
                    </div>
                </div>
    ...
    return (
        <div className="max-w-5xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start lg:items-center justify-between gap-6">
                <div>
                    <h1 className="text-3xl md:text-4xl lg:text-5xl font-black text-white tracking-tighter uppercase flex items-center gap-4">
                        <UserPlus size={32} className="text-primary lg:w-10 lg:h-10" />
                        <span>Intelligence <span className="text-primary">Capture</span></span>
                    </h1>
                    <p className="text-[11px] font-black text-muted-foreground uppercase tracking-[0.4em] mt-3 ml-1 leading-relaxed">
                        Manual data ingestion terminal for new entities
                    </p>
                </div>

                <div className="flex items-center gap-3 bg-card border border-border px-5 py-3 rounded-2xl w-fit">
                    <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
                    <span className="text-[10px] font-black text-foreground uppercase tracking-widest">Mainframe Active</span>
                </div>
            </div>
                ...
                <div className="lg:col-span-4 space-y-6">
                    <div className="bg-card border border-border rounded-[32px] p-8 shadow-premium">
                        <h3 className="text-[12px] font-black text-white uppercase tracking-widest mb-6 flex items-center gap-2">
                            <Sparkles size={16} className="text-primary" /> Protocol Rules
                        </h3>
                        <ul className="space-y-4">
                            {[
                                "Identity must be verified manually",
                                "All contact protocols must be numeric",
                                "Custom fields provide enhanced context",
                                "Avoid duplicate entity registration"
                            ].map((rule, idx) => (
                    ...
                    <div className="bg-primary/5 border border-primary/10 rounded-[32px] p-8">
                        <h3 className="text-[12px] font-black text-primary uppercase tracking-widest mb-4">Security Advisory</h3>
                        <p className="text-[10px] font-medium text-muted-foreground leading-relaxed">
                            Captured data is subject to automatic classification and workflow routing based on your priority settings.
                        </p>
                    </div>
                </div>
                        ...
                        <div className="relative z-10 space-y-8">
                            <AnimatePresence>
                                {lastCapturedLead && (
                                    ...
                                    >
                                        ...
                                        <h3 className="text-2xl font-black text-white uppercase tracking-tighter mb-2">Lead Synchronized</h3>
                                        <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-widest mb-8 max-w-[280px]">
                                            {lastCapturedLead.name} has been added to the intelligence stream.
                                        </p>
                                        <div className="flex flex-col w-full gap-3">
                                            <button
                                                type="button"
                                                onClick={() => handleDownloadPDF(lastCapturedLead)}
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => setLastCapturedLead(null)}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] ml-1">Entity Identity</label>
                                ...
                                {/* Email Input */}
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] ml-1">Comms Protocol</label>
                                    <div className="relative group/input">
                                        <div className={`absolute left-5 top-1/2 -translate-y-1/2 transition-colors duration-300 ${errors.email ? 'text-rose-500' : 'text-muted-foreground group-focus-within/input:text-primary'}`}>
                                            <Mail size={18} />
                                        </div>
                                        <input
                                            type="email"
                                            ...
                                            className={`w-full bg-secondary/50 border rounded-2xl pl-14 pr-5 py-4 text-[12px] font-black uppercase tracking-widest focus:outline-none transition-all duration-300 ${errors.email ? 'border-rose-500/50 focus:border-rose-500' : 'border-border focus:border-primary/50'}`}
                                        />
                                    </div>
                                ...
                                {/* Contact Input */}
                                <div className="space-y-3">
                                    <label className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em] ml-1">Direct Signature</label>
                                    <div className="relative group/input">
                                        <div className={`absolute left-5 top-1/2 -translate-y-1/2 transition-colors duration-300 ${errors.contact ? 'text-rose-500' : 'text-muted-foreground group-focus-within/input:text-primary'}`}>
                                            <Phone size={18} />
                                        </div>
                                        <input
                                            type="text"
                                            ...
                                            className={`w-full bg-secondary/50 border rounded-2xl pl-14 pr-5 py-4 text-[12px] font-black uppercase tracking-widest focus:outline-none transition-all duration-300 ${errors.contact ? 'border-rose-500/50 focus:border-rose-500' : 'border-border focus:border-primary/50'}`}
                                        />
                                    </div>
                                    {errors.contact && <p className="text-[9px] font-black text-rose-500 uppercase tracking-widest ml-1">{errors.contact}</p>}
                                </div>
                            </div>
                            ...
                            <div className="pt-6 border-t border-border/50">
                                <div className="flex items-center justify-between mb-8">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <button
                                            ...
                                            title="Add Custom Intelligence Parameter"
                                        >
                                            <Plus size={16} />
                                ...
                                {fetchingFields ? (
                                    <div className="flex items-center gap-3 p-6 bg-secondary/20 rounded-2xl animate-pulse">
                                        ...
                                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Fetching Dynamic Intelligence Fields...</span>
                                    </div>
                                ) : customFields.length === 0 ? (
                                    <p className="text-[10px] font-bold text-muted-foreground/40 uppercase tracking-widest text-center py-4 italic">
                                        No custom intelligence parameters defined
                                    </p>
                                ) : (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="pt-8">
                                ...
                                <p className="text-center text-[9px] font-bold text-muted-foreground uppercase tracking-widest mt-5 opacity-60">
                                    By synchronizing, you confirm this data adheres to the global classification protocol.
                                </p>
                            </div>
                        </div>
                    </form>
                    ...
                    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
                        ...
                        >
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center justify-between mb-8">
                                    <div>
                                        <h3 className="text-xl font-black uppercase tracking-tight text-white">Add intelligence field</h3>
                                        <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest mt-1 opacity-60">
                                            Define new telemetry mapping protocol
                                        </p>
                                    </div>
